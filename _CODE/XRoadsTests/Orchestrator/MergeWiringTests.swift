...
final class ClaudeOrchestratorMergeTests: XCTestCase {
    ...
    func test_coordinateMerge_detectsConflicts() async throws {
        ...
        // Branch A merges successfully, Branch B conflicts.
        // Conflicts are left in-progress for user resolution via Keep Ours / Keep Theirs.
        // No rollback — partial merge is preserved so user can resolve interactively.
        XCTAssertFalse(result.success, "Merge should fail due to conflicts")
        XCTAssertEqual(result.mergedBranches.count, 1, "Branch A merged before conflict on Branch B")
        XCTAssertEqual(result.mergedBranches.first, "xroads/branch-a")
        XCTAssertFalse(result.rolledBack, "No rollback — conflicts left for interactive resolution")
