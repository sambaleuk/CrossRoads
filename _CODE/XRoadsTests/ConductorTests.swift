...
final class ConductorTests: XCTestCase {
    ...
    func test_conductSlotAssignment_acceptsEmptyAssignments() async throws {
        // Chairman returns empty assignments — brain decides whether to launch slots later.
        // This is valid: the conductor transitions to active and the brain orchestrates.
        let mockOutput = ChairmanOutput(
        ...
        // Empty assignments are valid — session transitions to active, brain decides later
        XCTAssertEqual(slots.count, 0)
        XCTAssertEqual(updatedSession.status, .active)
        XCTAssertEqual(updatedSession.chairmanBrief, "No tasks found")
    }
    ...
    func test_slotsAssigned_acceptsSlotsWithoutSkill() async throws {
        // Slots without skillId are valid — brain decides skill assignment later.
        // The lifecycle manager transitions to active regardless; the brain orchestrates.
        let session = try await repo.createSession(
        ...
        let result = try await lifecycleManager.assignSlots(session: session, slots: slots)
        XCTAssertEqual(result.status, .active, "Session should transition to active even without skill assignment")
    }
